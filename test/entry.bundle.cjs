...
__export(entry_exports, {
  ...
  isSyncablePath: () => isSyncablePath,
  parseDocList: () => parseDocList,
...
var Plugin = class {
  constructor() {
    this.manifest = { dir: ".obsidian/plugins/cloud-relay", version: "test" };
  }
  async loadData() {
    return {};
  }
...
var writeBinaryEncoder = (encoder2, append) => writeUint8Array(encoder2, toUint8Array(append));
...
var readUpdate = (decoder2, ydoc, transactionOrigin) => readUpdateV2(decoder2, ydoc, transactionOrigin, new UpdateDecoderV1(decoder2));
...
var readID = (decoder2) => createID(readVarUint(decoder2), readVarUint(decoder2));
...
var readRelativePosition = (decoder2) => {
  let type = null;
  let tname = null;
  let itemID = null;
  switch (readVarUint(decoder2)) {
    case 0:
      itemID = readID(decoder2);
      break;
    case 1:
      tname = readVarString(decoder2);
      break;
    case 2: {
      type = readID(decoder2);
    }
  }
  const assoc = hasContent(decoder2) ? readVarInt(decoder2) : 0;
...
var encodeSnapshotV2 = (snapshot2, encoder2 = new DSEncoderV2()) => {
  writeDeleteSet(encoder2, snapshot2.ds);
  writeStateVector(encoder2, snapshot2.sv);
  return encoder2.toUint8Array();
};
var encodeSnapshot = (snapshot2) => encodeSnapshotV2(snapshot2, new DSEncoderV1());
var decodeSnapshotV2 = (buf, decoder2 = new DSDecoderV2(createDecoder(buf))) => {
  return new Snapshot(readDeleteSet(decoder2), readStateVector(decoder2));
};
...
var createDocFromSnapshot = (originDoc, snapshot2, newDoc = new Doc()) => {
  ...
  originDoc.transact((transaction) => {
    ...
    writeVarUint(encoder2.restEncoder, size2);
    for (const [client, clock] of sv) {
      ...
      writeVarUint(encoder2.restEncoder, lastStructIndex + 1);
      encoder2.writeClient(client);
      writeVarUint(encoder2.restEncoder, 0);
  ...
  applyUpdateV2(newDoc, encoder2.toUint8Array(), "snapshot");
  return newDoc;
};
  ...
  if (curr !== null) {
    ...
    for (; curr !== null; curr = updateDecoder.next()) {
      if (currClient !== curr.id.client) {
        if (currClock !== 0) {
          size2++;
          writeVarUint(encoder2.restEncoder, currClient);
          writeVarUint(encoder2.restEncoder, currClock);
        }
    ...
    writeBinaryEncoder(enc, encoder2.restEncoder);
    encoder2.restEncoder = enc;
    return encoder2.toUint8Array();
  } else {
    writeVarUint(encoder2.restEncoder, 0);
    return encoder2.toUint8Array();
...
var MSG_DOC_LIST = 0;
var MSG_SYNC_STEP1 = 1;
var MSG_SYNC_STEP2 = 2;
var MSG_UPDATE = 3;
var MSG_PING = 254;
var MSG_PONG = 255;
...
function encodeFrame(type, noteId, payload) {
  const id2 = encoder.encode(noteId);
  const frame = new Uint8Array(3 + id2.length + payload.length);
  frame[0] = type;
  frame[1] = id2.length >> 8 & 255;
  frame[2] = id2.length & 255;
  frame.set(id2, 3);
  frame.set(payload, 3 + id2.length);
  return frame;
}
...
var HIDDEN_DIRS = ["themes", "snippets"];
function isHiddenSyncable(path) {
  const parts = path.split("/");
  return HIDDEN_FILES.includes(parts[parts.length - 1]) || HIDDEN_DIRS.includes(parts[0]);
}
var skipNoticeShown = /* @__PURE__ */ new Set();
async function sha256Hex(buf) {
  const digest = await crypto.subtle.digest("SHA-256", buf);
  return Array.from(new Uint8Array(digest)).map((b) => b.toString(16).padStart(2, "0")).join("");
}
var PERSIST_DEBOUNCE_MS = 3e3;
var NoteSyncManager = class {
  constructor(app, vault, store) {
    this.app = app;
    this.vault = vault;
    this.store = store;
    this.index = {};
    this.docs = /* @__PURE__ */ new Map();
    this.svCache = /* @__PURE__ */ new Map();
    this.applyingRemoteByPath = /* @__PURE__ */ new Set();
    this.conn = null;
    this.suspended = false;
    this.pendingPush = /* @__PURE__ */ new Set();
    this.applySerial = Promise.resolve();
    this.persistTimers = /* @__PURE__ */ new Map();
    ...
    this.selfWritesObsidian = /* @__PURE__ */ new Map();
  ...
  startFlushLoop() {
    ...
    this.flushTimer = window.setInterval(() => {
      for (const id2 of this.persistTimers.keys()) {
        void this.persistNow(id2);
      }
    }, 2e3);
  }
  stopFlushLoop() {
  ...
  async init(showProgress = false) {
    ...
    for (const id2 of noteIds) {
      if (!this.index[id2].deleted) {
        const sv = await this.store.readSv(id2);
        if (sv) this.svCache.set(id2, sv);
      }
      ...
      if (idx.mtime !== file.stat.mtime) {
        ...
        if (entry) {
          ...
          if (content !== entry.lastContent) {
            const docLooksStale = entry.lastContent.length === 0 && content.length > 0 && idx.mtime !== 0;
            if (docLooksStale) {
              console.warn(
  ...
  attachMap() {
    ...
    return entry ? entry.doc.getMap("files") : null;
  }
  markLocallyDeletedAttachments() {
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (seen && seen.mtime === file.stat.mtime) {
          const entry2 = map.get(file.path);
          if (!entry2 || entry2.sha !== seen.sha || entry2.deleted) {
            map.set(file.path, {
              sha: seen.sha,
              size: file.stat.size,
              deleted: false
  ...
  onAttachmentChange(file, deleted = false, oldPath) {
    if (this.suspended) return;
  ...
  async initHiddenFiles(showProgress = false) {
    ...
    const map = this.docMap(HIDDEN_ID);
    if (!map) return;
    let i = 0;
    ...
    for (const f of files) {
      ...
      try {
        ...
        if (seen && seen.mtime === f.mtime) {
          const entry2 = map.get(f.path);
          if (entry2 && entry2.sha === seen.sha && !entry2.deleted) continue;
        }
  ...
  onHiddenFileChange(path, deleted = false) {
  ...
  markSelfWriteObsidian(path, mtime) {
    this.selfWritesObsidian.set(path, { mtime, until: Date.now() + 5e3 });
  }
  isSelfWriteObsidian(path) {
  ...
  async reconcileHiddenFromRemote() {
    if (this.suspended) return;
    const map = this.docMap(HIDDEN_ID);
  ...
  async reconcileAttachmentsFromRemote() {
    if (this.suspended) return;
    const map = this.attachMap();
    if (!map || !this.http) return;
    for (const [path, meta] of map.entries()) {
      ...
      if (meta.deleted) {
        if (this.attachSeen[path]) {
          const file2 = this.vault.getAbstractFileByPath(path);
          if (file2 instanceof TFile) {
            let mtime = 0;
            try {
              mtime = file2.stat.mtime;
            } catch {
            }
            this.applyingRemoteByPath.add(path);
            try {
              await this.vault.trash(file2, true);
            } catch {
              try {
                await this.vault.delete(file2);
              } catch {
              }
            }
      ...
      if (file instanceof TFile && file.stat.size === meta.size && this.attachSeen[path]?.sha === meta.sha) {
        continue;
      }
      try {
        const buf = await this.downloadBlob(meta.sha);
  ...
  async flushPendingPush(conn) {
    if (this.pendingPush.size === 0) return;
    const ids = Array.from(this.pendingPush);
    this.pendingPush.clear();
    for (const id2 of ids) {
      if (this.index[id2]?.deleted) continue;
      await this.ensureDoc(id2, this.index[id2]?.path ?? "");
      const entry = this.docs.get(id2);
      if (!entry) continue;
      const update = encodeStateAsUpdate(entry.doc);
      conn.send(encodeFrame(MSG_UPDATE, id2, update));
    }
  }
  suspend() {
    this.suspended = true;
  ...
  async sendSyncSteps(conn) {
    ...
    for (const id2 of ids) {
      ...
      if (!sv) {
        ...
        const entry = this.docs.get(id2);
        if (entry) {
          sv = new Uint8Array(encodeStateVector(entry.doc));
          this.svCache.set(id2, sv);
        }
      }
      if (sv && sv.length > 0 && conn) {
        conn.send(encodeFrame(MSG_SYNC_STEP1, id2, sv));
      }
    }
  }
  async onDocList(noteIds) {
    for (const id2 of noteIds) {
      if (!this.index[id2]) this.index[id2] = { path: "", deleted: false };
      let sv = this.svCache.get(id2);
      if (!sv) {
        await this.ensureDoc(id2, this.index[id2].path);
        const entry = this.docs.get(id2);
        sv = entry ? new Uint8Array(encodeStateVector(entry.doc)) : new Uint8Array(0);
        this.svCache.set(id2, sv);
      }
      if (this.conn && sv.length > 0) {
        this.conn.send(encodeFrame(MSG_SYNC_STEP1, id2, sv));
      }
    }
    this.scheduleIndexWrite();
    this.scheduleInitAttachments();
  }
  scheduleInitAttachments() {
    if (this.attachmentsInitRunning) {
      this.attachmentsInitPending = true;
      return;
    }
    this.attachmentsInitRunning = true;
    void this.initAttachments().catch(() => {
    }).finally(() => {
      this.attachmentsInitRunning = false;
  ...
  onSyncStep1(noteId, sv) {
    void (async () => {
      await this.ensureDoc(noteId, this.index[noteId]?.path ?? "");
      const entry = this.docs.get(noteId);
      if (!entry || !this.conn) return;
      const diff = encodeStateAsUpdate(entry.doc, sv);
      this.conn.send(encodeFrame(MSG_SYNC_STEP2, noteId, diff));
    })();
  }
  onSyncStep2(noteId, update) {
    this.queueApply(noteId, update);
  }
  onUpdate(noteId, update) {
    this.queueApply(noteId, update);
  }
  queueApply(noteId, update) {
    this.applyQueueDepth++;
    this.applySerial = this.applySerial.then(() => this.applyRemote(noteId, update)).catch((e) => console.error("cloud-relay apply gagal:", e)).finally(() => {
      this.applyQueueDepth--;
    });
  }
  applyQueueSize() {
  ...
  onFileModify(file, content) {
    ...
    this.index[id2].mtime = file.stat.mtime;
    this.scheduleIndexWrite();
    void (async () => {
      await this.ensureDoc(id2, file.path);
      const entry = this.docs.get(id2);
      if (!entry) return;
      if (content === entry.lastContent) {
        this.pushFullStateIfUnknown(id2);
        return;
      }
      const idx = this.index[id2];
      if (!isNewNote && entry.lastContent.length === 0 && content.length > 0 && idx && idx.mtime !== 0) {
        idx.mtime = file.stat.mtime;
        this.scheduleIndexWrite();
        return;
      }
  ...
  onFileRename(file, oldPath) {
    ...
    if (this.applyingRemoteByPath.has(oldPath) || this.applyingRemoteByPath.has(file.path))
      return;
  ...
  async applyRemote(noteId, update) {
    ...
    await sleep0();
    if (noteId === ATTACH_ID) {
      let entry2 = this.docs.get(ATTACH_ID);
      if (!entry2) {
        await this.ensureDoc(ATTACH_ID, "");
        entry2 = this.docs.get(ATTACH_ID);
      }
      if (!entry2) return;
      entry2.doc.transact(() => {
    ...
    if (noteId === HIDDEN_ID) {
      let entry2 = this.docs.get(HIDDEN_ID);
      if (!entry2) {
        await this.ensureDoc(HIDDEN_ID, "");
        entry2 = this.docs.get(HIDDEN_ID);
      }
      if (!entry2) return;
      entry2.doc.transact(() => {
        applyUpdate(entry2.doc, update);
    ...
    entry.doc.transact(() => {
      applyUpdate(entry.doc, update);
    }, "remote");
    const newContent = entry.text.toString();
    const newPath = entry.meta.get("path") ?? "";
  ...
  async ensureDoc(noteId, path) {
    ...
    doc.on("update", (update, origin) => {
      if (origin !== "remote") {
        if (this.conn) {
          const full = encodeStateAsUpdate(entry.doc);
    ...
    this.svCache.set(noteId, new Uint8Array(encodeStateVector(doc)));
  }
  persistDoc(noteId) {
    const prev = this.persistTimers.get(noteId);
    if (prev !== void 0) window.clearTimeout(prev);
    const t = window.setTimeout(() => {
      void this.persistNow(noteId);
    }, PERSIST_DEBOUNCE_MS);
    this.persistTimers.set(noteId, t);
  }
  async persistNow(noteId) {
    this.persistTimers.delete(noteId);
    const entry = this.docs.get(noteId);
    if (!entry) return;
    const blob = encodeStateAsUpdate(entry.doc);
    const sv = new Uint8Array(encodeStateVector(entry.doc));
  ...
  diagnostic() {
    const localNoteIds = [];
    const pathById = {};
    for (const [id2, idx] of Object.entries(this.index)) {
      if (id2 === ATTACH_ID || id2 === HIDDEN_ID) continue;
      if (!idx.deleted && idx.path) {
        localNoteIds.push(id2);
        pathById[id2] = idx.path;
      }
    }
  ...
  findNoteIdByPath(path) {
    for (const [id2, entry] of Object.entries(this.index)) {
      if (entry.path === path && !entry.deleted) return id2;
    }
    return null;
  }
...
function diffText(oldS, newS) {
  let s = 0;
  const min2 = Math.min(oldS.length, newS.length);
  while (s < min2 && oldS[s] === newS[s]) s++;
  let o = oldS.length;
  let n = newS.length;
...
function buildInviteLink(settings) {
...
var MockAdapter = class {
  ...
  async stat(path) {
    ...
    if (!f) return null;
    return { mtime: f.mtime, size: f.data.byteLength, type: "file" };
  }
  async list(path) {
  ...
  async rename(from2, to) {
    const f = this.files.get(from2);
    if (!f) throw new Error(`ENOENT ${from2}`);
    this.files.delete(from2);
    this.files.set(to, f);
  }
};
var MockVault = class {
...
var STALE_THRESHOLD_MS = 35e3;
var OUTGOING_BUFFER_MAX = 500;
var RelayConnection = class {
  constructor(onStatus, handlers) {
    this.ws = null;
  ...
  connect(serverUrl, vaultId, token) {
    this.closedByUser = false;
  ...
  scheduleReconnect(serverUrl, vaultId, token) {
